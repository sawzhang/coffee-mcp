...
| **鉴权** | 简单 Bearer Token 透传 | HMAC-SHA256 / SM2 签名（MCP 层实现签名逻辑） |
...
```
方案：B2B 客户在 MCP 连接时提供 appKey + appSecret，MCP Adapter 层负责签名。
...
│    "headers": {                              │
│      "X-SBUX-App-Key": "partner_app_key",    │
│      "X-SBUX-App-Secret": "partner_secret"   │  ← 只传一次
│    }                                         │
│  }                                           │
└──────────────────┬───────────────────────────┘
                   │ MCP Protocol (SSE)
...
│  MCP Adapter Layer                            │
│                                              │
│  1. 从 MCP 连接头提取 appKey + appSecret       │
│  2. 每次 Tool 调用时：                         │
│     a. 构造 X-Date（当前 GMT 时间）             │
│     b. 构造 Digest（SHA-256 请求体摘要）        │
│     c. 计算 HMAC-SHA256 签名                   │
│     d. 组装 Authorization 头                   │
│  3. 发送到 openapi.starbucks.com.cn            │
...
|----------|----------|------|------|------|
| `member_register` | 会员注册 | POST | `/crmadapter/account/register` | "帮这个用户注册星巴克会员" |
| `member_query` | 会员查询 | POST | `/crmadapter/account/query` | "查一下这个手机号有没有星巴克会员" |
| `member_bind` | 三方绑定 | POST | `/crmadapter/account/3pp/bind` | "把蔚来账号和星巴克会员绑定" |
...
| `member_login_url` | 登录链接 | POST | `/crmadapter/account/3pp/getLoginUrl` | "生成星巴克会员登录页面链接" |
...
| MCP Tool | HTTP API | 方法 | 路径 | 场景 |
|----------|----------|------|------|------|
| `equity_send` | 发放电子券 | POST | `/equity/send` | "给用户发一张星巴克券" |
...
| `srkit_send` | 权益发放(v2) | POST | `/srkit/send` | "发放星巴克权益，未注册会员自动挂起" |
...
| `order_push` | 全渠道推单 | POST | `/order/order_info/omni` | "推送一笔订单到星巴克" |
...
```
车主对蔚来 AI 说："到目的地前帮我在星巴克下单一杯拿铁"

蔚来 Agent MCP 调用链：
  1. member_query(mobile=车主手机号)     → 确认有星巴克会员
...
蔚来 MCP 配置:
{
  "mcpServers": {
    "starbucks": {
      "url": "https://mcp.starbucks.com.cn/sse",
      "headers": {
        "X-SBUX-App-Key": "nio_partner_key",
        "X-SBUX-App-Secret": "nio_partner_secret"
      }
    }
  }
}
...
千问 MCP 配置:
{
  "mcpServers": {
    "starbucks": {
      "url": "https://mcp.starbucks.com.cn/sse",
      "headers": {
        "X-SBUX-App-Key": "qwen_enterprise_key",
        "X-SBUX-App-Secret": "qwen_enterprise_secret"
      }
    }
  }
}
